...
#define OCT_NORM_PIVOT           1.0f/20.0f
...
int one_third_filter(unsigned int n, float f0, float* b, float* a) {
    ...
    assert(alpha > 1.0f);
    float fc = f0 * alpha;
    float m = iirdes_freqprewarp(LIQUID_IIRDES_BANDPASS, 0.5f * fc , f0);
    printf("m : %12.8f\n", m);
...
unsigned construct_freq_set(float* freq_set, unsigned* pivot, float sampling_rate)
{
    ...
    if (sampling_rate < 3 * band_edges[OCT_EDGE_FREQS - 1]) {
        ...
        while (freq_set[f_index] < sampling_rate/3.0f) {
            freq_set[f_index] = freq_set[f_index] / sampling_rate;
            f_index++ ;
        }
    ...
    // after decimation.
    unsigned p_index = 0;
    while ((p_index < f_index) && (freq_set[p_index] < OCT_NORM_PIVOT)) {
        p_index++;
    }
